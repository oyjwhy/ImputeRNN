...
class imputegru(object):
    ...
    def RNN(self,X, M, Delta, Lastvalues, X_lengths,Keep_prob, reuse=False):
         ...
         with tf.variable_scope("imputegru", reuse=reuse):
            ...
            # state = array_ops.concat([states_fw[:, 0:self.n_hidden_units], states_bw[:, 0:self.n_hidden_units]], 1)
    ...
    def test(self):
        ...
        while epochcount<self.epoch:
            ...
            epochcount+=1
            dataset.shuffle(self.batch_size,True)
            for x,y,x_,m_cover,m_mse,m,deltaPre,x_lengths,lastvalues,deltaSub,subvalues in dataset.nextBatch():
                _,cross_entropy_loss,impute_loss,combine_loss,summary_str,acc,pred,outputs_ = self.sess.run([self.train_op,self.cross_entropy, self.impute_loss, self.combine_loss,self.sum, self.accuracy,self.pred,self.outputs], feed_dict={\
                    self.x: x,\
                    self.y: y,\
                    self.m: m,\
                    self.delta: deltaPre,\
                ...
                if counter%10==0 or counter==28:
                    try:
                        auc = metrics.roc_auc_score(np.array(y),np.array(pred))
                    except ValueError:
                        pass
            ...
            #     if epochcount % 10 == 0:
            #         save_imputation_y.extend(y)
            #         imputed=np.array(1-np.array(m)) * np.array(outputs_) + np.array(m) * np.array(x)
            ...
            for x,y,x_,m_cover,m_mse,m,deltaPre,x_lengths,lastvalues,deltaSub,subvalues in testdataset.nextBatch():
                acc,pred,rmse,mae,outputs_ = self.sess.run([self.accuracy,self.pred,self.rmse,self.mae,self.outputs], feed_dict={\
                    self.x: x,\
                    self.y: y,\
                    self.x_: x_,\
                    self.m_mse: m_mse,\
            ...
            #     if epochcount % 10 == 0:
            #         save_test_imputation_y.extend(y)
            #         imputed=np.array(1-np.array(m_cover)) * np.array(outputs_) + np.array(m_cover) * np.array(x)
