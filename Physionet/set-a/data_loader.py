...
class MyDataset(data.Dataset):
    def __init__(self, dataPath, labelPath):
        ...
        dic={'time':0,'Age':1,'Gender':2,'Height':3,'ICUType':4,'Weight':5,'Albumin':6,\
             ...
             'SaO2':35,'SysABP':36,'Temp':37,'TropI':38,'TropT':39,'Urine':40,'WBC':41,'TroponinI':42,'TroponinT':43}
...
dataset = MyDataset("./train/", "./list.txt")

train_loader = torch.utils.data.DataLoader(\
    dataset, batch_size=1, shuffle=False)
print(len(train_loader))
# print(train_loader)
for dataseta, filename, label in train_loader:
#     # print(dataseta)
    print(filename)
    print(dataseta.size())
    print(label)
